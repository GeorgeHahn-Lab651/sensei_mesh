...
#define BSP_INIT_NONE                                                          \
  0 /**< This define specifies the type of initialization without support for  \
       LEDs and buttons (@ref bsp_init).*/
#define BSP_INIT_LED                                                           \
  (1 << 0) /**< This bit enables LEDs during initialization (@ref bsp_init).*/
#define BSP_INIT_BUTTONS                                                       \
  (1 << 1) /**< This bit enables buttons during initialization (@ref           \
              bsp_init).*/
 ...
 */
typedef enum {
  BSP_EVENT_NOTHING = 0, /**< Assign this event to an action to prevent the
                            action from generating an event (disable the
                            action). */
 ...
 *
 * @param[in]   p_buttons_state          This variable will store buttons state.
 * Button 0 state is
 *                                       represented by bit 0 (1=pressed),
 * Button 1 state by bit 1,
 *                                       and so on.
 *
 ...
 * @param[in]   p_state                  This variable will store the
 * information whether the
 *                                       specified button is pressed (true) or
 * not.
 *
 * @retval      NRF_SUCCESS              If the button state was successfully
 * read.
 */
 ...
 * @retval      NRF_ERROR_INVALID_STATE   If the application timer module has
 * not been initialized,
 *                                        or internal timer has not been
 * created.
 */
...
/**@brief       Function for configuring wakeup buttons before going into sleep
 * mode.
 *
 * @details     After calling this function, only the buttons that are set to 1
 * in wakeup_buttons
 *              can be used to wake up the chip. If this function is not called
 * before going to,
 ...
 *
 * @param[in]   wakeup_buttons  Mask describing which buttons should be able to
 * wake up the chip.
 *
 * @retval      NRF_SUCCESS              If the buttons were successfully
 * enabled.
 ...
 *
 * @details     This function must be called before going to sleep, otherwise
 * NFCT will not be
 *              able to wake up the chip.
 * @note        You can use this function only if NFCT is not used for other
 * purposes than
 *              wakeup.
 *
 * This function should only be called immediately before going into sleep.
 *
 * @retval      NRF_SUCCESS              If NFCT peripheral was successfully
 * enabled.
 * @retval      NRF_ERROR_NOT_SUPPORTED  If the board has no NFCT peripheral or
 * NRF52 is not defined.
 */
